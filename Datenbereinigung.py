...
for row in reader:
    row[1] = row[1].replace(";",",")#ersetzt evtl. auftauchende ";", um frühzeitige Zelltrennung zu vermeiden
    ...
    if row[10] != "True":#Entfernt gekürzte tweeets
                        writer.writerow([row[0],row[1],row[2],row[3],row[4],row[5],row[7],row[8]])#schreibt alle ungekürzten in die out_file
